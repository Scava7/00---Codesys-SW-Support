from Modules.TXT_Generator.constants import pers_vars_bool, pers_vars_int, PLC, HMI, netvar_par_bool, netvar_par_int
...
def actual_par_block(df, header_title, device):
    ...
    def formatta_riga(row):
        ...
        if device == PLC:
            ...
            if header_title == "PAR BOOL":
                return f"{group} Upd_BOOL[{row['n']}] (ptIO := ADR({glob}){glob_spaces}, ptPers := ADR({support_var})); //{unit} {desc}"
            else:
                return f"{group} Upd_INT[{row['n']}] (ptIO := ADR({glob}){glob_spaces}, ptPers := ADR({support_var})); //{unit} {desc}"
...
########################################################################################################################################
def actual_par_block_new(df, header_title):
    # Formatta i campi di base
    df["group_fmt"] = df["group"].apply(lambda g: f"(*{g}*)")
    max_len_group = df["group_fmt"].str.len().max()
    ...
    def formatta_riga(row):
        ...
        if header_title == "PAR BOOL":
            return f"{group} Upd_BOOL[{row['n']}] (ptIO := ADR({glob}){glob_spaces}, uiCfgArray := {row['n']}); //{unit} {desc}"
        else:
            return f"{group} Upd_INT[{row['n']}] (ptIO := ADR({glob}){glob_spaces}, uiCfgArray := {row['n']}); //{unit} {desc}"
